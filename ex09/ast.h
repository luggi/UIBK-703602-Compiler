...
/* true if given node uses body for other nodes */
bool node_uses_body(const struct ast_node *node);

/* walks through the tree starting at node and calls f on each node */
void node_walk(struct ast_node *node, void (*f)(struct ast_node *));
